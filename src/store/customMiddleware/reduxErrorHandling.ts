...
import { ICwaError, StatusCode } from './ICwaError'
import { httpRequestHandler } from '../../utils/api/httpRequestHandler';
import { ERROR_LOGGING } from '../../utils/api/urlConstants';

export const reduxErrorHandlingMiddleware = (store: any) => (next: Dispatch) => (action: any) => {
    try {
        return next(action)
    } catch (err) {
...
export const reduxApiErrorHandlingMiddleware = (store: any) => (next: Dispatch) => (action: any) => {
    if (action.type.includes("FAILURE")) {
        ...
        httpRequestHandler.postRequest(ERROR_LOGGING, data)
        console.error("A error occur while calling an external resource via Redux thunk", data);
        return next(action);
    } else {
        return next(action);
    }
